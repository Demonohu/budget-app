...
class Budget:
    def __init__(self):
        """ Initialize the categories and the starting budgets of each categories to 0. """
    ...
    def deposit(self, amount, category):
        """ The deposit method; takes in the amount and category, and adds it to the category balance. It returns an error message if the user puts a wrong category. """
    ...
    def withdraw(self, amount, category):
        """ The withdraw method; takes in the amount and category, checks if there is up to that amount in the category  balance and removes the amount from the balance if there's sufficient funds. It returns an error message if there's insufficient funds. """
        if category.lower() == 'food':
            if self.food_balance >= amount:
                self.food_balance -= amount
            else:
                return "You don't have the facilities for that, werey."
        elif category.lower() == 'entertainment':
    ...
    def transfer(self, amount, sender, receiver):
        """ The transfer method; takes in the amount, sender(category) and receiver(category), checks if there's sufficient fund in the sender's balance and transfers the amount to the receiver's balance if there's sufficient funds. It returns an error message if otherwise. """
        if sender.lower() == 'food' and receiver.lower() == 'entertainment':
            ...
            return "Check wetin you input. You don make mistake."

    def get_balance(self, category):
        """ The get balance method; takes in the category and returns the balance. """
        if category.lower() == 'food':
            return '${:,.2f}'.format(self.food_balance)
        elif category.lower() == 'entertainment':
            return '${:,.2f}'.format(self.entertainment_balance)
        elif category.lower() == 'clothing':
            return '${:,.2f}'.format(self.clothing_balance)